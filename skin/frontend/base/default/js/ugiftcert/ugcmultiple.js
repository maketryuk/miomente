...
//    }
    ...
    if (window.ugcAllowScheduled) {
        var calJson = window.ugcCalendarJson;
        calJson.inputField = calJson.inputField+'-1';
        calJson.button = calJson.button+'-1';
        Calendar.setup(calJson);
    }
...
//        MyTabs.removeTabTitles('h5.tab-title');
    ...
    function insertTab(container, content) {
        ...
        if (window.ugcAllowScheduled) {
            var calJson = window.ugcCalendarJson;
            calJson.inputField = calJson.inputField+'-'+current;
            calJson.button = calJson.button+'-'+current;
            Calendar.setup(calJson);
        }
        return tab;
    }
    ...
    function updateTabElements(tab, incrementId) {
        var elements = tab.select('input,textarea,img.calendar-trig');
