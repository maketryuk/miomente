(function ($, window, document, undefined) {
    $.widget("infortis.ultramegamenu", {
        ...
        _initPlugin: function () {
            ...
            if (this.options.outermostContainer === undefined)this.options.outermostContainer = this.bar; else if (this.options.outermostContainer === "window") {
                this.options.outermostContainer = $(window);
                this.outermostContainerIsWindow = true
            }
            if (this.options.fullWidthDdContainer === undefined)this.options.fullWidthDdContainer = this.bar; else if (this.options.fullWidthDdContainer === "window") {
        ...
        _initDualMode: function () {
            ...
            enquire.register("screen and (max-width: " + (this.options.mobileMenuThreshold - 1) + "px)", {
                ...
                deferSetup: true, setup: function () {
                    _self._cleanUpAfterMobileMenu()
                }, match: function () {
        ...
        hookToMobnavTriggerClick: function () {
            ...
            this.mobnavTrigger.on("click", function (e) {
                ...
                if ($(this).hasClass("active"))_self.closeMenuViaMobnavTrigger(); else _self.openMenuViaMobnavTrigger()
            });
        ...
        _showDropdown: function (item) {
            //$('.nav-panel--dropdown').parent('li').addClass('active');
            item.children("a.level-top").addClass('active');
            ...
            var modX = 0;
            var modY = 0;
            if (isVert)modX = item.outerWidth(); else modY = item.height();
            var ddPos = {left: itemPos.left + modX, top: itemPos.top + modY};
            ...
            var menubarOffset;
            if (this.outermostContainerIsWindow === false)menubarOffset = menubar.offset().left - outermostCon.offset().left; else menubarOffset = menubar.offset().left;
            var ddOffset = menubarOffset + ddPos.left;
            if (dd.hasClass("full-width"))if (isVert) {
        ...
        _hideDd: function (item) {
            //console.log('hide');
            item.children("a.level-top").removeClass('active');
            $( "#dim" ).hide();
            var _self = this;
