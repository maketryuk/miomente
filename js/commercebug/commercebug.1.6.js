...
jQueryCommercebug.fn.commercebug = function(l_commercebug_json) {
    ...
    //set up default objects for anything that's missing
    //until we can figure out what's really going on
    //with full page cache
    var top_level = ['models','blocks'];
    $.each(top_level, function(key, value){
        if(!l_commercebug_json_parsed[value])
        {
            l_commercebug_json_parsed[value] = {};
        }
    });
    ...
    function action_lookup()
    {
        ...
        if(val)
        {
            ...
            if(collected_data.isMage2)
            {
                action = 'lookupClassMage2';
            }
            params['form_key'] = collected_data.system_info.form_key;
            // $('#ascommercebug_lookup_results').load(collected_data.system_info.ajax_paths[action],params);//?uri=catalog%2Fnavigation');             
            $('#ascommercebug_lookup_results').load(collected_data.system_info.ajax_paths[action],params);
        }
    }
    ...
    function getNonceFromUrl(url)
    {
        ...
        var found_key   = false;
        var nonce       = '';
        jQueryCommercebug.each(parts, function(index, part){
            if(found_nonce)
            {
                return;
            }
    ...
    function init_404_tab(pElement)
    {
        var collected_data  = l_commercebug_json_parsed; 
        var labels          = l_commercebug_json_parsed.labels.controllers;
        var status_404      = l_commercebug_json_parsed.status_404;
        ...
        table.setId('ascommercebug_404_table');        
        table.addHeader(labels.label).addHeader(labels.value);
        table.addGlobalClass('table','tablesorter');
        
        table.addRow(['Original Path'       , status_404.originalUrlPath]);
        table.addRow(['Module/Front Name'   , status_404.urlModuleName]);
        
        var diagnoses = [];
        var html = '';
        
        if(status_404.claimedByName)
        {
            table.addRow(['Claimed By'          , status_404.claimedByName]);
        }
        else
        {
            diagnoses.push('<p>No modules claim [<code>'+status_404.urlModuleName+'</code>] as a <code>&lt;frontName/&gt;</code> </p>');
        }
        
        table.addRow(['Controller Name'     , status_404.urlControllerName]);
        
        if(status_404.controllerFileExists)
        {            
            table.addRow(['Controller Class'    , '<code>' + status_404.controllerClassName + '</code>']);
            table.addRow(['Controller Path'     , '<span class="pathinfo">' + status_404.controllerFilePath + '</span>']);
        }
        else
        {
            html = '<p>The controller name [<code>';
            html += status_404.urlControllerName;
            html += '</code>] matches the following file, but this file does not exist.'
            html += '<br/><textarea style="width:100%">'+status_404.controllerFilePath+'</textarea>';
            diagnoses.push(html);
        }
                
        if(status_404.actionMethodExists)
        {
            table.addRow(['Action Name'         , status_404.urlActionName]);
        }
        else
        {
            html = 'The action method <code>';
            html += status_404.urlActionName;
            html += 'Action</code> is not present in the controller file.';
            diagnoses.push(html);
        }
        
        // table.addRow(['Original Path','...']);
        diagnoses.push('<p>MVC routing appears to be correct.</p>');
        
        diagnoses = diagnoses.shift();
        
        var extra = '';        
        if(status_404.extraModules && status_404.extraModules.length > 0)
        {
            extra = '<p>This Front Name is also claimed by the following modules, but [<code>'+status_404.claimedByName+'</code>] won.</p>';
            extra += '<textarea style="width:100%;height:4em;">'; 
            extra += status_404.extraModules.join("\n") + '</textarea>';
        }
        $('#ascommercebug_404').html(table.render() + diagnoses + extra);                
    }
    ...
    {        
        ...
        cssHeader:'ascommercebug_tablesort_header'});        
        }                
    ...
    function init_models_other(pElement)
    {
        ...
        $.each(files.mage2_code, function(key, value){
            process_file_row(value, table, 'mage2');
        });
    ...
    {    
        ...
        $.each(files, function(key, value){            
            ...
            else if(value.indexOf('app/code/') === 0)
            {
                info.mage2_code.push(value.substr('app/code'.length+1));
            }
            else{
    ...
    function init_models_crud(pElement)
    {
        ...
        // console.log(l_commercebug_json_parsed);
    ...
    function init_layout(pElement)
    {
        ...
        results.push('<p>');
        results.push('<a href="#" id="as_commercebug_display_dot_link"> '+labels.view_graphviz+' </a>');
        results.push('| <a href="#" id="as_commercebug_render_graphviz"> '+labels.render_graphviz+' </a>');
        results.push('</p>');        
        results.push('<form style="display:block" target="_blank" method="post" id="as_commercebug_render_graphviz_form" action="');
        results.push(l_commercebug_json_parsed.layout.url_render_dot);
        results.push('">');
        ...
        results.push('<input name="token_as_commercebug" type="hidden" value="');
        results.push(l_commercebug_json_parsed.nonce_cross_system);
        results.push('"/>')
        results.push('</form>');
        results.push(table.render());

        if(!l_commercebug_json_parsed.isMage2)
        {
            ...
            if(design_paths['parent_theme'])
            {
                results = push_cellpair(results,'Parent',design_paths['parent_theme']);
            }
        ...
        //jQueryCommercebug('#as_commercebug_render_graphviz_form').submit()
        $('#as_commercebug_render_graphviz').click(function(){
            $('#as_commercebug_render_graphviz_form').submit();
    ...
    function init_system_tab(pElement)
    {
        ...
        if(!l_commercebug_json_parsed.isMage2)
        {
            ...
            content += '<button id="ascommercebug_but_toggle_translation" class="fg-button ui-state-default ui-corner-all">'+labels.toggle_translate+'</button>'        
        }
        content += '</p>' + "\n";
        ...
        $('#ascommercebug_but_clear_cache').bind('click', function()
        {
            ...
            $('#ascommercebug_clear_cache_results').load(collected_data.system_info.ajax_paths['clearcache']);
        });        
        
        $('#ascommercebug_but_togglehints').bind('click', function()
        {
            $('#ascommercebug_clear_cache_results').html('toggling ...');
            $('#ascommercebug_clear_cache_results').load(collected_data.system_info.ajax_paths.togglehints);
        });        

        $('#ascommercebug_but_toggleblockhints').bind('click', function()
        {
            $('#ascommercebug_clear_cache_results').html('toggling ...');
            $('#ascommercebug_clear_cache_results').load(collected_data.system_info.ajax_paths['toggleblockhints']);
        });                

        $('#ascommercebug_but_toggle_magelogging').bind('click', function()
        {
            $('#ascommercebug_clear_cache_results').html('toggling ...');
            $('#ascommercebug_clear_cache_results').load(collected_data.system_info.ajax_paths['togglemagelogging']);
        });            

        $('#ascommercebug_but_toggle_cblogging').bind('click', function()
        {
            $('#ascommercebug_clear_cache_results').html('toggling ...');
            $('#ascommercebug_clear_cache_results').load(collected_data.system_info.ajax_paths['togglecblogging']);
        });   
        
        $('#ascommercebug_but_toggle_translation').bind('click', function()
        {
            $('#ascommercebug_clear_cache_results').html('toggling ...');
            $('#ascommercebug_clear_cache_results').load(collected_data.system_info.ajax_paths['toggletranslation']);        
        });
    ...
    function init_collections(pElement)
    {
        ...
        var collections         = o.collections ? o.collections : {};            
        var collectionFiles     = o.collectionFiles ? o.collectionFiles : {};                
        var collectionModels    = o.collectionModels ? o.collectionModels : {};        
...
//         table.push('<\/table>');
    ...
    init_tabs(this);    
    init_event_handlers(this); //Commerce Bug jquery event handlders
